...
export async function GET(request: Request) {
  ...
  // 检查认证是否启用
  if (!config.enabled) {
    return NextResponse.redirect(new URL('/auth-error?auth_error=disabled', PUBLIC_URL))
  }

  // 验证配置
  if (!validateGitLabConfig(config)) {
    return NextResponse.redirect(new URL('/auth-error?auth_error=config', PUBLIC_URL))
  }

  // 处理错误
  if (error) {
    return NextResponse.redirect(new URL(`/auth-error?auth_error=${error}`, PUBLIC_URL))
  }

  if (!code) {
    return NextResponse.redirect(new URL('/auth-error?auth_error=no_code', PUBLIC_URL))
  }
  ...
  try {
    ...
    return NextResponse.redirect(new URL('/auth-error?auth_error=server_error', PUBLIC_URL))
  }
}
